...
// Function to generate Gaussian noise matrix for a window of epochs
MatrixXd generateGaussianNoiseMatrix(int seed, int nEpoch, const MatrixXd &covariance)
{
...
// Function to generate Pearson type IV noise matrix for a window of epochs
MatrixXd generatePearsonNoiseMatrix(int seed, int nEpoch,
                                    const VectorXd &means, const VectorXd &stds,
                             ...
                             bool usePearsonNoise)
{
    if (usePearsonNoise)
    {
        // Get the square root of the diagonal elements of the covariance matrix
        VectorXd stds = covariance.diagonal().array().sqrt();
        return generatePearsonNoiseMatrix(seed, nEpoch, means, stds, skews, kurts);
    }
    else
    {
        return generateGaussianNoiseMatrix(seed, nEpoch, covariance);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    // // header for the saved file
    // vector<string> headerTraj({"tSec", "x", "y", "z", "vx", "vy", "vz"});
    // string trajTruthFile = snrInfo.outDir + "/trajectory_truth";
    // trajTruthFile += ".csv";
