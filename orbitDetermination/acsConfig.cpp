	...
	boost::program_options::variables_map&	newCommandOpts)	///< Variable map object of command line options
{
	...
	auto output_files = stringsToYamlObject({yaml, ""}, 		{"output_files"});
	{
		...
		trySetEnumOpt(clocks_receiver_source,	output_files, {"clocks_receiver_source" }, E_ClkDataSrc::_from_string_nocase);
		trySetEnumOpt(clocks_satellite_source,	output_files, {"clocks_satellite_source"}, E_ClkDataSrc::_from_string_nocase);
