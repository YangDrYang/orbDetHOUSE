...
Eigen::Vector3d calculateDragForce(const Eigen::Vector3d& rSat, 
    ...
    Eigen::Vector3d drag = -0.5 * rho * Area * Cd * pow(V.norm(), 1) * V;
...
double calculateDensity(double t, const Eigen::VectorXd& X, IERS iersInstance){
    ...
    // <lat, long, alt>
    std::vector<double> position = cartesianGeodetic(X, t, iersInstance);
...
std::vector<double> cartesianGeodetic(Eigen::VectorXd Xeci, double t, IERS iersInstance) {
    Eigen::VectorXd Xecef(6);
    ...
    std::vector<double> returnVal{latitude, longitude, altitude};
    return returnVal;
}
