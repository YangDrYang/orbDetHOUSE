...
struct ObsKey
{
	...
	{
		char buff[100];
		snprintf(buff, sizeof(buff), "%5s\t%3s\t%5s%d", str.c_str(), Sat.id().c_str(), type.c_str(), num);
		string str = buff;
...
namespace std
{
	...
	struct hash<ObsKey>
	{
		size_t operator()(ObsKey const &key) const
		{
			...
			size_t hashval = hash<string>{}(key.str) << 0 ^ hash<size_t>{}(key.Sat) << 1 ^ hash<string>{}(key.type) << 2 ^ hash<int>{}(key.num) << 3;
			return hashval;
		}
	};
}
...
namespace std
{
	...
	struct hash<KFKey>
	{
		size_t operator()(KFKey const &key) const
		{
			...
			size_t hashval = hash<string>{}(key.str) << 0 ^ hash<size_t>{}(key.Sat) << 1 ^ hash<int>{}(key.type) << 2 ^ hash<short>{}(key.num) << 3;
			return hashval;
		}
	};
}
