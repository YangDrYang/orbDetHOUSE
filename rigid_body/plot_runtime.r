...
rt_gauss <- as.matrix(read.csv("out/run_time_gauss.csv"))[,1:5]
rt_pearson <- as.matrix(read.csv("out/run_time_pearson.csv"))[,1:5]

fil <- c("HOUSE", "UKF", "CUT-4", "CUT-6", "CUT-8")

filter <- rep(fil, 2)
Distributions <- c(rep("Gaussian Distributions", 5),
                   rep("Pearson Type IV Distributions", 5))
...
ggsave("rb_runtime.eps", width=3.25, height=3.5, units="in")
