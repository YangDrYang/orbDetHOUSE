...
process_rmse <- function() {
    ...
    for (j in 1:trials) {
        ...
        tru_file <- paste("out/tru_",  j, ".csv", sep="")

        house_file <- paste("out/house_", j, ".csv", sep="")
        ukf_file   <- paste("out/ukf_",  j, ".csv", sep="")
        # cut4_file  <- paste("out/cut4_est_",   "_", j, ".csv", sep="")
        # cut6_file  <- paste("out/cut6_est_",   "_", j, ".csv", sep="")
    ...
    num_orbits = 20
    n = 10
    rmse_pos <- c(sqrt(mean(tail(house_r_se,    n * length(house_r_se) %/% num_orbits))),
                  sqrt(mean(tail(ukf_r_se,      n * length(ukf_r_se) %/% num_orbits))))
                #   sqrt(mean(tail(cut4_r_se,     n * length(cut4_r_se) %/% num_orbits))))
                #   sqrt(mean(cut6_r_se)))

    rmse_vel <- c(sqrt(mean(tail(house_v_se,    n * length(house_v_se) %/% num_orbits))),
                  sqrt(mean(tail(ukf_v_se,      n * length(ukf_v_se) %/% num_orbits))))
                #   sqrt(mean(tail(cut4_v_se,     n * length(cut4_v_se) %/% num_orbits))))
                #   sqrt(mean(cut6_v_se)))
...
process_box <- function() {
    ...
    for (j in 1:trials) {
        ...
        tru_file <- paste("out/tru_", "_",  j, ".csv", sep="")

        house_file <- paste("out/house_",  "_", k, ".csv", sep="")
        ukf_file   <- paste("out/ukf_",    "_", k, ".csv", sep="")
        # cut4_file  <- paste("out/cut4_est_",   "_", k, ".csv", sep="")
        # cut6_file  <- paste("out/cut6_est_",   "_", k, ".csv", sep="")
    ...
    pos_file <- paste("out/pos_err_", ".csv", sep="")
    vel_file <- paste("out/vel_err_", ".csv", sep="") 
