...
rmse_pearson <- as.matrix(read.csv("out/out_projectile/rmse_t_pearson.csv"))

steps <- 1000

dist <- c(
    rep("Gaussian Distributions", 8 * steps),
    rep("Pearson Type IV Distributions", 8 * steps)
)

comp <- rep(c(
    rep("Position RMSE (m)", steps),
    rep("Velocity RMSE (m/s)", steps)
), 8)
...
ggplot(data, aes(x = t, y = RMSE)) +
    geom_line(aes(col = Filter, linetype = Filter)) +
    scale_color_manual(values = colors) +
    facet_grid(
        rows = vars(comp), cols = vars(dist),
