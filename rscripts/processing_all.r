...
process_rmse <- function(trialNum, outDir) {

    print("Post-processing RMSE")

    tru_file <- paste(outDir, "/trajectory_truth.csv", sep="")
    tru <- as.matrix(read.csv(tru_file))[,2:7]
    steps <- nrow(tru)

    house_r_se <- matrix(, nrow=steps, ncol=trialNum)
    ukf_r_se   <- matrix(, nrow=steps, ncol=trialNum)
    cut4_r_se  <- matrix(, nrow=steps, ncol=trialNum)
    cut6_r_se  <- matrix(, nrow=steps, ncol=trialNum)

    house_v_se <- matrix(, nrow=steps, ncol=trialNum)
    ukf_v_se   <- matrix(, nrow=steps, ncol=trialNum)
    cut4_v_se  <- matrix(, nrow=steps, ncol=trialNum)
    cut6_v_se  <- matrix(, nrow=steps, ncol=trialNum)

    for (j in 1:trialNum) {

        print(paste("   Trial ", j), quote=FALSE)

        house_file <- paste(outDir, "/house_", j, ".csv", sep="")
        ukf_file   <- paste(outDir, "/ukf_",  j, ".csv", sep="")
        cut4_file  <- paste(outDir, "/cut4_", j, ".csv", sep="")
        cut6_file  <- paste(outDir, "/cut6_", j, ".csv", sep="")

        house_est <- as.matrix(read.csv(house_file))[,2:7]
        ukf_est   <- as.matrix(read.csv(ukf_file  ))[,2:7]
        cut4_est  <- as.matrix(read.csv(cut4_file ))[,2:7]
        cut6_est  <- as.matrix(read.csv(cut6_file ))[,2:7]
    ...
    file <- paste(outDir,  "/rmse.csv", sep="")
    ...
    file <- paste(outDir, "/rmse_t.csv", sep="")
    write.csv(rmse_t, file, row.names=FALSE, quote=FALSE)

    # --- Convergence

    convmse <- function(se) {
        cmse <- vector(, length = trialNum)
        for (j in 1:trialNum) {
            cmse[j] <- mean(se[,1:j])        
        }
        return(cmse)
    ...
    conv <- data.frame(ntrials = 1:trialNum,
    ...
    file <- paste(outDir, "/conv.csv", sep="")
    write.csv(conv, file, row.names = FALSE, quote=FALSE)

}

process_box <- function(trialNum, outDir) {
    ...
    pos_file <- paste(outDir, "/pos_err_", ".csv", sep="")
    vel_file <- paste(outDir, "/vel_err_", ".csv", sep="") 
...
process_rmse(trialNum, outDir)
process_box(trialNum, outDir)
